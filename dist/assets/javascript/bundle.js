...
search_bar.addEventListener("keyup", function () {
    for (var i = 0; i < items.length; i++) {
        if (items[i].querySelector("h2.item--content--title").innerHTML.toUpperCase().lastIndexOf(search_bar.value.toUpperCase()) == -1) items[i].style.display = "none";else if (search_bar.value === "") items[i].style.display = "block";else items[i].style.display = "block";
    }
});
...
function fetch_data() {
    fetch('https://launchlibrary.net/1.2/launch/' + id[timeline_index].value).then(function (response) {
        ...
        console.log(result.launches[0].location.pads[0].agencies);
